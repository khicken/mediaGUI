...
class VideoConcatenationWorker(QThread):
    ...
    def __init__(self, video_files, output_path, param, output_fps=30, mode=0):
        super().__init__()
        ...
        if mode == 0:
            self.frames_per_video = param if param else 30
        elif mode == 1:
            self.frame_step = param if param else 1
    ...
    def extract_x_frames(self):
        try:
            ...
            out = cv2.VideoWriter(str(self.output_path), fourcc, output_fps, (width, height))
            if not out.isOpened():
